...
func (b *Broker) StartConsuming(consumerTag string, concurrency int, appId string, taskProcessor iface.TaskProcessor) (bool, error) {
	b.Broker.StartConsuming(consumerTag, concurrency, appId, taskProcessor)
	...
	if err := b.consume(deliveries, concurrency, appId, taskProcessor, amqpCloseChan); err != nil {
		return b.GetRetry(), err
	}
...
func (b *Broker) consume(deliveries <-chan amqp.Delivery, concurrency int, appId string, taskProcessor iface.TaskProcessor, amqpCloseChan <-chan *amqp.Error) error {
	...
	for {
		...
		case d := <-deliveries:
			...
			go func() {
				if err := b.consumeOne(d, appId, taskProcessor, true); err != nil {
					errorsChan <- err
				}
...
func (b *Broker) consumeOne(delivery amqp.Delivery, appId string, taskProcessor iface.TaskProcessor, ack bool) error {
	fmt.Println("-----delivery.AppId-----", delivery.AppId)
	fmt.Println("-----appId-----", appId)
	if delivery.AppId != appId {
		// delivery.Nack(true, true) // multiple, requeue
		delivery.Recover(true)
		return errors.New("Received an mismatch message,appId " + delivery.AppId)
	}
...
func (b *Broker) GetPendingTasks(queue, appId string) ([]*tasks.Signature, error) {
	if queue == "" {
		queue = b.GetConfig().DefaultQueue
	}
	...
	for i := 0; i < queueInfo.Messages; i++ {
		...
		tag = d.DeliveryTag
		b.consumeOne(d, appId, dumper, false)
	}
